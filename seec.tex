...
High Performance Computing (HPC) systems tend to favor a race-to-idle approach to complete applications as quickly as possible.
Historically, this approach has resulted in reasonably energy-efficient systems, however modern systems are more energy-proportional.
...
We propose two projects to address balancing application performance and system power consumption in HPC \emph{during application runtime}.
...
\textbf{Dynamic voltage and frequency scaling} (DVFS) has been available on computing systems for roughly two decades now.
Despite the HPC preference for race-to-idle, DVFS is available in HPC job schedulers to allow applications to save energy by scaling back the processor during I/O or on code that is off the critical path.

We propose to use a machine learning-based classifier to predict the most energy-efficient DVFS settings at runtime to maximize the work-to-cost ratio of an application running on a single node.
This approach allows us to maximize the work-to-cost ratio of an application running on a single node.
To train and drive our classifier, we capture low-level hardware counter metrics, available through tools like PAPI or PCM.
Then at regular time intervals, our classifier predicts the most energy-efficient state to use based on measured application behavior and applies the setting to the system.
...
In more recent years, hardware has taken more direct control of voltage and frequency, making fine-tuned software-management of DVFS obsolete.
Fine-grained \textbf{power capping} is now the preferred mechanism for managing performance and power tradeoffs, evidenced by the introduction of power capping implementations like Intel Running Average Power Limit (RAPL) and the move from Speed Step to Speed Shift.
...
We propose a generalized multiple input, multiple output (MIMO) controller to shift power allocations between nodes to eliminate tail idle times in job iterations, thus maximizing application throughput while respecting a global power cap.

{
\PAD
\centering
  \input{img/scheme-nlpb.tex}
  \captionof{figure}{\interface{MIMO} proportional integral derivative (PID) closed loop controller.}
  \label{fig:runtime-nlpb}
\PAD
}

Given cluster power cap $\Gamma$, the PID \textbf{controller} computes new power signals $U$ in each iteration $t$:
\begin{eqnarray}
U(t) = U(t-1) + K_I \cdot E(t)
\end{eqnarray}
where $K_I$ is the ratio of control change and $E(t)$ are node idle times, normalized around their mean.
...
Unlike heuristic approaches to power management, control theory provides formal \emph{convergence guarantees} and is \emph{robust} to application, system, and measurement noise.
We simulate meeting a global power cap of 180 Watts on a quad-socket system using a MPI+OpenMP application, with each MPI process bound to a socket:
