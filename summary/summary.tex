...
Additionally, we can provide an optimal balance of performance and energy consumption.
...
\figref{runtime-ee} demonstrates our machine learning-based feedback design.
...
As applications move through \emph{phases}, the controller adapts to the changing application behavior.

\figref{classifier-hipmer-ee} demonstrates the controller behavior with the HiPMeraculous genome assembly application.
We trained the classifier from characterizations of HPC applications, like those from the NAS Parallel Benchmarks \cite{NPB}.
The bold horizontal line in the upper figure is the race-to-idle (nominal DVFS) frequency, and in the lower figure represents the average energy efficiency of the race-to-idle frequency.

\begin{figure}[t]
  \input{img/hipmer-ee.tex}
  \caption{Energy-efficient HiPMeraculous using a Gradient Boosting classifier.}
...
\figref{nlpb-sim} simulates meeting a global power cap of 180 Watts on an artificially imbalanced quad-socket system ($\Gamma=180$, $n=4$) using a MPI+OpenMP application, with each MPI process bound to a socket.
It begins with evenly distributed power caps, \ie $u_i(0) = \frac{\Gamma}{n}$ for each socket $i$.
